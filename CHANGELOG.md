...
- Fixed Crabfleet workspaces to use any configured brokered provider and route the OpenClaw deployment through its verified AWS backend with isolated, ephemeral key-only SSH access, stock-image cloud-init, and readiness-gated, pinned, Workers-compatible terminal attachment.
